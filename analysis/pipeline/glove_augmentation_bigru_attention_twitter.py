"""
Augmentation -> Pretrained GloVe -> Bidirectional GRU -> Attention -> Dense -> Softmax
Twitter financial sentiment dataset (zeroshot/twitter-financial-news-sentiment).

Uses the winning hyperparameters found in this round's sweeps:
  - dropout_rate  = 0.20   (best from the BiGRU dropout sweep)
  - learning_rate = 0.002  (best from the BiGRU hyperparameter sweep)
  - vocab_size    = 15000
  - rnn_units     = 128

NOTE on embedding_dim: the sweep found embedding_dim=128 best, but that was
for a *trainable, randomly-initialized* embedding layer. Pretrained GloVe
vectors only come in fixed sizes (50/100/200/300), so embedding_dim=128
isn't an option here. Of those, 100 is the closest available size to 128,
so this script uses GloVe's 100-dimensional vectors (glove.6B.100d.txt)
and keeps everything else from the tuned config as-is.
...
HP = {
    "learning_rate": 0.002,    # best from hyperparameter sweep
    "vocab_size": 15000,       # best from hyperparameter sweep
    "embedding_dim": 100,      # fixed by GloVe's available sizes (see note above)
    "rnn_units": 128,          # best from hyperparameter sweep
    "dropout_rate": 0.20,      # best from dropout sweep
    "max_len": 60,
}
...
# ==================================================================
# ATTENTION LAYER (simple additive/Bahdanau-style pooling over BiGRU outputs)
# ==================================================================
class AttentionPooling(tf.keras.layers.Layer):
    """
    Learns a weight for each timestep of the BiGRU output, then returns a
...
# ==================================================================
# MODEL — Augmentation (done earlier, on raw text) -> GloVe -> BiGRU -> Attention -> Dense -> Softmax
# ==================================================================
...
if __name__ == "__main__":
    ...
    # Step 4: Build Augmentation -> GloVe -> BiGRU -> Attention -> Dense -> Softmax
    model = build_model(
    ...
    print("\n" + "=" * 60)
    print("FINAL RESULTS — Augmentation -> GloVe -> BiGRU -> Attention -> Dense -> Softmax")
    print("=" * 60)
