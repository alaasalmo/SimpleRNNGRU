...
# Fixed hyperparameters — used as-is for BOTH GRU and BiGRU below.
DEFAULTS = {
    "learning_rate": 0.001,
    "vocab_size": 6000,
    "embedding_dim": 64,
...
# ==================================================================
# MODEL — GRU or BiGRU, selected via `bidirectional` flag
# ==================================================================
def build_model(vocab_size, embedding_dim, rnn_units, learning_rate, bidirectional):
    """
    bidirectional=False -> plain GRU (reads left-to-right only)
    bidirectional=True  -> Bidirectional(GRU) (reads both directions,
...
# ==================================================================
def plot_results(results, filename="gru_vs_bigru_twitter.png"):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    save_path = os.path.join(script_dir, filename)

    plt.figure(figsize=(9, 5))
    colors = {"GRU": "#2a78d6", "BiGRU": "#d6572a"}
    ...
    plt.title("GRU vs BiGRU — Validation Combined Score per Epoch\nTwitter Finance Dataset")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    gru_result = run_one_variant(
        tr_texts, tr_labels, val_texts, val_labels, test_texts, test_labels,
        vectorizer, DEFAULTS, bidirectional=False, label="GRU"
    )

    bigru_result = run_one_variant(
        tr_texts, tr_labels, val_texts, val_labels, test_texts, test_labels,
        vectorizer, DEFAULTS, bidirectional=True, label="BiGRU"
    )
    ...
    # ---------------------------------------------------------
    print("\n" + "=" * 70)
    print("GRU vs BiGRU — Comparison (fixed hyperparameters)")
    print("=" * 70)
