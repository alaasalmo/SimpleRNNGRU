...
SEED = 42
EPOCHS = 30      # up to 30 epochs, with early stopping via PATIENCE below
...
def load_data(csv_path="all-data.csv", test_size=0.15):
    ...
    Since this CSV has no separate train/test split (unlike the HF
    Twitter dataset used previously), one is carved out here with the
    same stratified, fixed-seed approach used for train/val elsewhere
    in this script.
    """
    candidates = [
        csv_path,
...
def print_label_distribution(labels, name):
    """
    Prints label counts using CLASS_NAMES (e.g. 'Negative') along with
