"""
THREE EXPERIMENTS — SimpleRNN sentiment classifier
====================================================

1) Dropout sweep: try 3 dropout values, find best epoch for each, plot all 3.
...
DROPOUT_RATES_TO_TEST = [0.2, 0.45, 0.6]   # experiment 1
...
if __name__ == "__main__":
    ...
    for rate in DROPOUT_RATES_TO_TEST:
        tracker = train_and_evaluate(
            ...
            run_name=f"Dropout={rate}"
        )
        dropout_results.append((f"dropout={rate}", tracker))

    plot_comparison(
        dropout_results,
        title="Experiment 1: Macro-F1 by Dropout Rate\n(stars = each curve's best epoch)",
    ...
    best_dropout = float(best_label.split("=")[1])
    print(f"\nBest dropout: {best_dropout} (macro-F1={best_tracker.best_f1:.4f})")
    ...
    # ============================================================
    print(f"\n{'='*60}\nFINAL OVERALL SUMMARY\n{'='*60}")
    print(f"Best dropout (Exp 1):        {best_dropout}")
