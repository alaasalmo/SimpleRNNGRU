...
HP = {
    # Values below are fixed from a prior hyperparameter search (BiGRU) over:
    #   learning_rate in [0.0005, 0.001, 0.002, 0.005]
    #   vocab_size    in [6000, 10000, 15000]
    #   embedding_dim in [32, 64, 128]
    #   rnn_units     in [32, 64, 128]
    # This script holds them constant and sweeps DROPOUT_RATES only.
    "learning_rate": 0.002,
    "vocab_size": 6000,
    "embedding_dim": 128,
    "rnn_units": 32,
...
# ==================================================================
# DATA — Kaggle "all-data.csv" (Financial PhraseBank style)
# Format: no header, column 0 = label (neutral/negative/positive),
# column 1 = text. File is ISO-8859-1 encoded.
# ==================================================================
CSV_PATH = "all-data.csv"  # place this next to the script, or give a full path
LABEL_MAP = {"negative": 0, "neutral": 1, "positive": 2}


def load_data():
    df = pd.read_csv(CSV_PATH, encoding="ISO-8859-1", header=None, names=["label", "text"])
...
# ==================================================================
# MODEL — BiGRU with adjustable dropout
# ==================================================================
def build_model(dropout_rate):
    model = tf.keras.Sequential([
        tf.keras.layers.Embedding(HP["vocab_size"], HP["embedding_dim"]),
        tf.keras.layers.Bidirectional(tf.keras.layers.GRU(HP["rnn_units"])),
...
# ==================================================================
# RUN ONE EXPERIMENT: train BiGRU for EPOCHS with given dropout
# ==================================================================
...
# ==================================================================
def plot_results(results, filename="bigru_dropout_comparison_kaggle.png"):
    ...
    plt.title(f"BiGRU — Dropout Comparison (fixed {EPOCHS} epochs)\nKaggle Financial Phrasebank Dataset")
    plt.grid(alpha=0.3)
    plt.tight_layout()
