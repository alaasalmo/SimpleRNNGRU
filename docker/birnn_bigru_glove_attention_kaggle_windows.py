...
#  Dataset: Kaggle Financial PhraseBank (all-data.csv), read from --input
#
#  Usage:
#     python birnn_bigru_glove_attention_kaggle_windows.py --input /data/input --output /data/output --model-type 1     
#
#  --input must be a folder containing all-data.csv (no header, two columns:
#  sentiment, text -- sentiment is one of negative/neutral/positive).
#
#  MODEL_TYPE:
#    1 = Bidirectional SimpleRNN  (dropout=0.20, epochs=13, lr=0.0005, vocab=6000, rnn_units=32)
#    2 = Bidirectional GRU        (dropout=0.20, epochs=13, lr=0.0005, vocab=6000, rnn_units=32)
#
#  Both configs use embedding_dim=100 (GloVe's closest available size to the
#  tuned 128 -- GloVe only ships in 50/100/200/300 dimensions).
...
# ── ARGS ──────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="BiSimpleRNN/BiGRU + GloVe + Attention MultiWorker Training (Kaggle CSV)")
parser.add_argument("--input",       required=True, help="Input directory (must contain all-data.csv; also used for GloVe/NLTK cache)")
...
MODEL_CONFIGS = {
    1: {
        "name": "BiSimpleRNN",
        "epochs": 13,            # fixed - best epoch found earlier
        "learning_rate": 0.0005,
        "vocab_size": 6000,
        "embedding_dim": 100,     # GloVe's closest available size to the tuned 128
        "rnn_units": 32,
        "dropout_rate": 0.20,
        "max_len": 60,
    },
...
# ── STEP 1: Load dataset ──────────────────────────────────────────────────────
# Kaggle Financial PhraseBank dataset (all-data.csv), read from the external
# /input folder. File has NO header and two columns: sentiment, text
# (sentiment is one of: negative, neutral, positive). Typically stored as
# latin-1, not utf-8. Since this CSV has no separate train/test split, one
# is carved out here with a stratified, fixed-seed split.
log("\n[Step 1] Loading dataset…")

CSV_PATH = os.path.join(INPUT_DIR, "all-data.csv")
if not os.path.exists(CSV_PATH):
    raise SystemExit(
        f"all-data.csv not found at {CSV_PATH} — make sure it's placed in "
        f"the folder mounted as --input."
    )
...
all_labels = df["sentiment"].map(label_map).tolist()

train_texts, test_texts, train_labels, test_labels = train_test_split(
    all_texts, all_labels, test_size=0.15, stratify=all_labels, random_state=SEED
)
