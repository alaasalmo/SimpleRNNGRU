...
# ── ARGS ──────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="BiSimpleRNN MultiWorker Training")
parser.add_argument("--input",       required=True, help="Input directory (dataset cache)")
...
log(f"{'='*52}")
log(f"BiSimpleRNN  |  worker-{task_index}  ({'chief' if is_chief else 'worker'})")
log(f"{'='*52}")
log(f"[Config]   input  = {INPUT_DIR}")
log(f"[Config]   output = {OUTPUT_DIR}")

# ── STARTUP DELAY (non-chief only) ───────────────────────────────────────────
# Give the chief time to start its gRPC server before workers connect.
if not is_chief:
...
# If non-chief workers stay alive during model.predict(), TF expects them to
# participate in CollectiveReduceV2 ops. When they don't, the chief crashes
# with "heartbeat timeout / UnavailableError". Exiting now prevents that.
if not is_chief:
    log(f"\n[Worker {task_index}] Training complete — exiting cleanly.")
    log_file.close()
    sys.exit(0)

# ── STEP 6: Evaluate — chief only, on a NEW single-worker model ───────────────
# We must NOT call model.predict() on the distributed model — it still has
# collective ops attached. Instead, build a fresh model and copy the weights.
...
# Rebuild test_ds with batch size 32 (no longer need global batch)
test_ds_infer = (
...
with open(report_path, "w") as f:
    f.write("CLASSIFICATION REPORT  (BiSimpleRNN — MultiWorker)\n")
    f.write("─" * 52 + "\n")
    f.write(report + "\n")
...
log("\n" + "─" * 52)
log("CLASSIFICATION REPORT  (BiSimpleRNN — MultiWorker)")
log("─" * 52)
log(report)
...
with open(pred_path, "w") as pf:
    pf.write("SAMPLE PREDICTIONS  (BiSimpleRNN)\n" + "─" * 52 + "\n")
