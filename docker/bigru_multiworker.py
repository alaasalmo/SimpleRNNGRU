#  BiGRU — Financial Sentiment Analysis
#  Multi-Worker Mirrored Strategy
#  Dataset: zeroshot/twitter-financial-news-sentiment
#
#  Usage:
#    python bigru_multiworker.py --input /data/input --output /data/output
#
#  TF_CONFIG is injected via environment variable by the launch scripts.
...
# ── ARGS ──────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="BiGRU MultiWorker Training")
...
log(f"{'='*52}")
log(f"BiGRU  |  worker-{task_index}  ({'chief' if is_chief else 'worker'})")
log(f"{'='*52}")
...
# ── STEP 4: Build BiGRU model inside strategy scope ───────────────────────────
log("\n[Step 4] Building BiGRU model…")
...
# across all workers, causing UnavailableError when worker-1 disconnects.
if not is_chief:
...
# Do NOT call predict() on the distributed model — it still has collective ops.
# Build a new model, copy weights, then predict safely on one machine.
...
with open(report_path, "w") as f:
    f.write("CLASSIFICATION REPORT  (BiGRU — MultiWorker)\n")
    f.write("─" * 52 + "\n")
    f.write(report + "\n")
...
log("\n" + "─" * 52)
log("CLASSIFICATION REPORT  (BiGRU — MultiWorker)")
log("─" * 52)
log(report)
...
log("\n" + "─" * 52)
log("SAMPLE PREDICTIONS  (BiGRU)")
log("─" * 52)

with open(pred_path, "w") as pf:
    pf.write("SAMPLE PREDICTIONS  (BiGRU)\n" + "─" * 52 + "\n")
